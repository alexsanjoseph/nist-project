...
example parsed MTConnect Agent data and device XML files are provided along with the package. We will be using
...
The devices XML has information about the configuration of one or more devices. This
file can be built either from the `probe` request from a working MTConnect Agent or directly
from the configuration of the MTConnect Agent if you have access to it
...
## Getting Adapter log data by parsing Agent data

Alternately, data from an MTConnect Agent can be passed through a `ruby` parsed to get the parsed
agent data, which is used by the `mtconnectR` Package. 


## Raw Adapter Log data - Not Recommended

If you don't have the Agent data with you, you can use the Adapter Log data directly.
This method is not recommended because it is common for the Adapter Log to have various
errors and inconsistencies which might affect the integity of the analysis.
